const renderBlogData = (document, stackOrder) => {
  ...
  const deleteTemplate = ` ${(putanja != "/reportedStuff.html") ? `<div class='delete' >X</div>` : `<button class="deleteReportedPost">Delete this post</button>`} `;
  ...
  const blogTemplate = ` <li class="blog-list-element" id=${document.id}> 
    ...
    <span class="prikaziBlogChildren">${data.title}</span>
     <img  class="prikaziBlogChildren blogPicture" src="${data.picture != null ? data.picture : cat}" alt="#">
      ...
      ${(putanja != "/reportedStuff.html") ? `<p class="createdAt" onmouseover="toggleTimeCreated()" onmouseleave="toggleTimeCreated()"> ${timeAgo} </p>` : ""}
       
      ${(putanja != "/reportedStuff.html") ? `<p class="commentPost">Comment this post</p>` : ""}
